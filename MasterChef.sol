...
// MasterChef is the master of Brew Token (BREW). He can make BREW and he is a fair guy.
//
...
// contract once $BREW is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accBrewPerShare; // Accumulated BREWs per share, times 1e18. See below.
    ...
    // Initial emission rate: 0.02 BREW per block.
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed __BrewPerBlock);
    ...
    constructor(
        ...
        BrewPerBlock = INITIAL_EMISSION_RATE;
    ...
    function getMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {
        if (IERC20(brew).totalSupply() + totalLockedUpRewards  >= MAX_SUPPLY_CAP) {
            return 0;
        }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accBrewPerShare = accBrewPerShare.add(
                BrewReward.mul(1e18).div(lpSupply)
            );
        }
        uint256 pending =  user.amount.mul(accBrewPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest Brew's.
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accBrewPerShare).div(1e18);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Deposit LP tokens to MasterChef for brew allocation with referral.
    ...
    // Pay or lockup pending brew.
    function payOrLockupPendingBrew(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accBrewPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeBrewTransfer(msg.sender, totalRewards);
    ...
    function updateEmissionRate(uint256 _BrewPerBlock) public onlyOwner {
        massUpdatePools();
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            brew.mint(referrer, refBonusEarned);
